...
describe("catalog v2 settings and sync history", () => {
  it("persists runtime settings and retains only the newest 100 events per project", async () => {
    ...
    catalog.setRuntimeSettings({ closeToTray: false, onboardingCompleted: true, syncPaused: true });
    ...
    expect(catalog.runtimeSettings()).toEqual({ closeToTray: false, onboardingCompleted: true, syncPaused: true });
    expect(catalog.syncHistory("project-one", 200)).toHaveLength(100);
    catalog.close();
  });
});
