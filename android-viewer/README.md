...
一个只读的 GitHub LaTeX 项目浏览器。界面采用内容优先的 Liquid Glass 风格：浮动导航和阅读工具使用真实背景模糊，连续列表与 PDF 内容保持清晰，并跟随系统浅色/深色主题。Android 12 以下或低内存设备会自动使用高对比霜化材质，保证流畅与可读性。
...
- 主导航只保留“阅读 / 项目 / 下载”，设置从右上角进入；项目和文件采用扁平单列，长文件名支持两行显示。
- 下载进度面板可随时收起，底部“下载”入口会保留进行中状态点；详情操作使用靠近拇指区的底部面板。
- PDF 阅读器采用沉浸式纸张画布和按可见范围缓存；低内存时主动降级并释放位图，单页失败可重试，也可交给其他 PDF 应用打开。
...
- 项目行的更多菜单和项目文件页右上角菜单中的“下载项目 ZIP”会下载默认分支 ZIP。
